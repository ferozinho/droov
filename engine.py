...
class Engine:
    ...
    def temp_db(self):
        self.db = {"drivers": None, "consignments": None}
    ...
    def find_nearest_driver(self, consignment: Consignment) -> int:
        ...
        for k, v in drivers.items():
            if not v.status == DriverStatus.AVAILABLE or v.is_occupied:
                continue
            driver = v
    ...
    def assign_consignment(self, c_queue: ConsignmentQueue):
        ...
        consignment.logs.append(
            f"Consignment id:{consignment.c_id} has been assigned to {assigned_driver.name} at {consignment.updated_at}"
        )
    ...
    def pick_consignment(self, c_id):
        consignments = self.db["consignments"]
        consignment = consignments[c_id]
        ...
        consignment.updated_at = datetime.datetime.now()
        consignment.logs.append(
            f"Consignment id:{consignment.c_id} has been picked by {driver.name} at {consignment.updated_at}"
        )
    ...
    def start_delivery_process(self, c_id: int):
        ...
        consignment.updated_at = datetime.datetime.now()
        consignment.logs.append(
            f"Consignment id:{consignment.c_id} is being dispatched by {driver.name} at {consignment.updated_at}"
        )
        driver.status = DriverStatus.BUSY_DELIVERING

    def finish_delivery(self, c_id: int):
        consignments = self.db["consignments"]
        consignment = consignments[c_id]
        driver = self.db["drivers"][consignment.assigned_to]
        ...
        consignment.updated_at = datetime.datetime.now()
        consignment.logs.append(
            f"Consignment id:{consignment.c_id} has been delivered by {driver.name} at {consignment.updated_at}"
        )
        print(consignment.logs)
        consignment.is_active = False
        driver.consignments.remove(c_id)
        if not len(driver.consignments) == 0:
            self.start_delivery_process(driver.consignments[0])
